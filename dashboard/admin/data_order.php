                ...
                foreach ($order as $orders):
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <?= date('d-m-Y H:i', $orders['tanggal_order']) ?>
                            </td>
                            <td>Rp.
                                <?= rupiah($hartot['hartot']) ?>
                            </td>
                            <td>
                                <?= $orders['keterangan_order'] ?>
                            </td>
                            <td>
                                <a href="fungsi/hapusPesan.php?id=<?= $orders['id_order'] ?>"
                                    class="btn btn-danger btn-sm text-small"
                                    onclick="return confirm('Yakin ingin menghapus data ini ?')"><i
                                        class="fas fa-trash"></i></a>
                                <button type="button" title="Detail Order"
                                    class="btn btn-sm btn-secondary text-small text-white" data-toggle="modal"
                                    data-target="#detailOrder_<?= $orders['id_order'] ?>"><i
                                        class="fas fa-eye"></i></button>
                                <?php if ($orders['order_status'] == 1): ?>
                                    <a href="print_struk.php?id_order=<?= $orders['id_order'] ?>" target="_blank"
                                        class="btn btn-warning text-white btn-sm text-small"><i class="fas fa-print"></i></a>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<?php foreach ($order as $detRow): ?>
    <div class="modal fade" id="detailOrder_<?= $orders['id_order'] ?>" tabindex="-1" role="dialog"
                        ...
                        <tbody>
                            ...
                            foreach ($detail_order as $list_row):
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?= $list_row['keterangan_dorder'] ?>
                                    </td>
                                    <td>Rp.
                                        <?= rupiah($query_masakan['harga_masakan']) ?>
                                    </td>
                                    <td align="center">
                                        <?= $list_row['jumlah_dorder'] ?>
                                    </td>
                                    <td>Rp.
                                        <?= rupiah($query_masakan['harga_masakan'] * $list_row['jumlah_dorder']) ?>
                                    </td>
                                </tr>
                                <?php $i++; endforeach; ?>
                        </tbody>
