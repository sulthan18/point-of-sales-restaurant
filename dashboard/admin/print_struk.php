  ...
  <div class="container mt-5">
    <div class="row">
      <div class="col-md-6 mx-auto" style="border: 1px solid #000;">
        <h4 class="text-center mt-2"><img src="../assets/image/logo.png" width="40" align="middle"><span
            class="mt-3">Chicken House</span></h4>
        <hr>
        <div class="row mt-3">
          <div class="col-md-6">
            TANGGAL :
            <?= date('d-m-Y h:i', $struk['tanggal_transaksi']) ?><br>
            NO ORDER :
            <?= $struk['id_order'] ?>
          </div>
          <div class="col-md-6">
            Member :
            <?= $struk['id_user'] == 0 ? 'Tidak' : $mem['nama_user'] ?>
          </div>
        </div>
        <hr>
        <div class="row">
          <?php foreach ($detail_order as $do):
            ...
            <div class="col-md-4 text-right">Rp.
              <?= rupiah($masakan['harga_masakan']) ?>
            </div>
            <div class="col-md-4 text-right">Rp.
              <?= rupiah($do['hartot_dorder']) ?>
            </div>
          <?php endforeach; ?>
        </div>
        <hr>
        ...
        <div class="row">
          ...
          <div class="col-md-4 text-right">Rp.
            <?= rupiah($struk['kembalian_transaksi']) ?>
          </div>
        </div>
        <hr>
        <div class="row">
